    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(huodong !=null){
            ...
            BeanUtils.copyProperties( huodong , view );//把实体数据重构到view中
                ...
                //级联表
                HuodongguanliyuanEntity huodongguanliyuan = huodongguanliyuanService.selectById(huodong.getHuodongguanliyuanId());
                if(huodongguanliyuan != null){
                    BeanUtils.copyProperties( huodongguanliyuan , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setHuodongguanliyuanId(huodongguanliyuan.getId());
                }
            //修改对应字典表字段
    ...
    */
    ...
    public R save(@RequestBody HuodongEntity huodong, HttpServletRequest request){
        ...
        if(false)
            ...
            huodong.setHuodongguanliyuanId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<HuodongEntity> queryWrapper = new EntityWrapper<HuodongEntity>()
            .eq("huodongguanliyuan_id", huodong.getHuodongguanliyuanId())
            ...
            .eq("huodong_zuida_number", huodong.getHuodongZuidaNumber())
        ...
        if(huodongEntity==null){
            huodong.setZanNumber(0);
            huodong.setCaiNumber(0);
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            HuodongEntity huodongEntity = new HuodongEntity();
//                            huodongEntity.setHuodongguanliyuanId(Integer.valueOf(data.get(0)));   //活动管理员 要改的
//                            huodongEntity.setHuodongName(data.get(0));                    //组团活动名称 要改的
//                            huodongEntity.setHuodongUuidNumber(data.get(0));                    //组团活动编号 要改的
...
//                            huodongEntity.setHuodongTypes(Integer.valueOf(data.get(0)));   //组团活动类型 要改的
//                            huodongEntity.setHuodongZuidaNumber(Integer.valueOf(data.get(0)));   //最大参与人数 要改的
//                            huodongEntity.setHuodongClicknum(Integer.valueOf(data.get(0)));   //活动热度 要改的
//                            huodongEntity.setZanNumber(Integer.valueOf(data.get(0)));   //赞数量 要改的
//                            huodongEntity.setCaiNumber(Integer.valueOf(data.get(0)));   //踩数量 要改的
...
//                            huodongEntity.setCreateTime(date);//时间
                        ...
                        if(huodongEntities_huodongUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [组团活动编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
        ...
        HuodongEntity huodong = huodongService.selectById(id);
            if(huodong !=null){
                ...
                //级联表
                    HuodongguanliyuanEntity huodongguanliyuan = huodongguanliyuanService.selectById(huodong.getHuodongguanliyuanId());
                if(huodongguanliyuan != null){
                    BeanUtils.copyProperties( huodongguanliyuan , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setHuodongguanliyuanId(huodongguanliyuan.getId());
                }
                //修改对应字典表字段
                dictionaryService.dictionaryConvert(view, request);
